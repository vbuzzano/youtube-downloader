...
_download_videos() {
  ...
  if [ $COUNT -gt 0 ]; then
    ...
    echo "Copy downloaded videos to $DEST_DIR" && \
    cp -rfn * $DEST_DIR && \
    cd .. && \
    echo "Append downloaded archive to $DOWNLOAD_ARCHIVE and remove empty lines" && \
...
while IFS= read -r URL; do
  ...
  echo ""
  echo "Prepare to download videos for $URL ..."
  ...
  echo ""
  echo "Download new videos files for $URL"
